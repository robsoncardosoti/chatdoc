...
def main():
    ...
    with st.sidebar:
        ...
        with tab1:
            ...
            model_loader = st.selectbox('Selecione o carregador do modelo:', list_model_loader)
            llm_model = st.selectbox('Selecione o modelo:', list_llms(model_loader))
            uploaded_files = st.file_uploader("Selecione os arquivos e clique em Processar",accept_multiple_files=True, type=['pdf', 'doc', 'docx'])
        with tab2:
            ...
            with col2:
                chunk_type = st.selectbox('Segmentação:', ('Recursiva', 'Fixa', 'Tokens','Semântica', 'Sentença'))
                type_retriever = st.selectbox('Recuperador:', list_type_retriever)
        ...
        if st.button("Processar"):
            with st.spinner("Processando"):
                ...
                st.session_state.conversation = get_conversation_chain(vectorstore, llm_model, context_window,
                                                                       max_tokens, temperature, model_loader,type_retriever)
    ...
    print("Tempo de execução: ", time.time() - tempo_inicio)
